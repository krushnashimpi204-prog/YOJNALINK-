...
function setupAgreement() {
  ...
  agreementButton.addEventListener("click", () => {
    ...
    window.location.href = "login-c.html";
  });
}
...
function setupLoginForm() {
  ...
  verifyButton.addEventListener("click", () => {
    ...
    // ========================================================
    // BACKEND WORK HERE: Replace this localStorage role check
    // with a server-side authenticated /api/auth/me response.
    // The server MUST decide whether the user is the primary
    // admin, another admin, or a normal user.
    // ========================================================
    const adminAccounts = (() => {
      try { return JSON.parse(localStorage.getItem(ADMIN_ACCOUNTS_KEY) || "[]"); }
      catch { return []; }
    })();
    const phone = mobileInput.value;
    const isPrimaryAdmin = phone === ADMIN_PHONE;
    const otherAdmin = adminAccounts.find(a => a.phone === phone && a.active !== false);

    if (isPrimaryAdmin || otherAdmin) {
...
function setupInfoForm() {
  ...
  const aadhaarInput = document.querySelector("#profile-aadhaar");
  ...
  if (savedProfile) {
    ...
    aadhaarInput.value = savedProfile.aadhaar || "";
  }
  ...
  infoForm.addEventListener("submit", (event) => {
    ...
    const aadhaar = normalizeAadhaar(aadhaarInput.value);
    if (!/^[0-9]{12}$/.test(aadhaar)) {
      formMessage.textContent = "कृपया 12 अंकों का सही आधार नंबर दर्ज करें।";
      aadhaarInput.focus();
      return;
    }
    ...
    const profile = {
      ...
      phone: phone,
      // Demo only: production should keep the encrypted Aadhaar on the server.
      aadhaar: aadhaar,
    ...
    const finishSaving = () => {
      ...
      // BACKEND INTEGRATION: await fetch('/api/users/profile', {
    ...
    // दस्तावेज़ को data URL के रूप में सेव करें ताकि प्रोफाइल पेज पर दिखाया जा सके
    // (बड़ी फाइलों के लिए preview छोड़ दें, सिर्फ नाम/टाइप सेव करें)
    const MAX_PREVIEW_SIZE = 2 * 1024 * 1024; // 2MB
    if (documentFile.size <= MAX_PREVIEW_SIZE) {
      const reader = new FileReader();
      reader.onload = () => {
        profile.documentData = reader.result;
        finishSaving();
      };
      reader.onerror = () => {
        // Preview सेव नहीं हो पाई तो भी बाकी जानकारी सेव कर दें
        finishSaving();
      };
